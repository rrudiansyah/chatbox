...
# def main():
#     st.title("Chatbot FAQ dengan TF-IDF")
...
#         if user_input.strip():
#             faq_df = load_faq()
#             answer = chatbot_response(user_input, faq_df)
#             if answer:
#                 st.write("🤖 Bot:", answer)
#             else:
#                 st.write("🤖 Bot: Saya belum tahu jawaban untuk itu.")
#                 if st.button("Ajukan Pertanyaan Ini"):
#                     user_questions_df = load_user_questions()
#                     new_question = pd.DataFrame({"question": [user_input]})
#                     user_questions_df = pd.concat([user_questions_df, new_question], ignore_index=True)
...
#             new_question = st.text_input("Pertanyaan Baru")
#             new_answer = st.text_input("Jawaban Baru")
#             if st.button("Tambahkan ke FAQ"):
...
def main():
    ...
    # Tampilkan form untuk memasukkan pertanyaan
    user_input = st.text_input("Anda:", st.session_state.user_input)
    ...
    # Tombol "Tanya"
    if st.button("Tanya", key="tanya_button"):
        if st.session_state.user_input.strip():
            faq_df = load_faq()
            answer = chatbot_response(st.session_state.user_input, faq_df)
            if answer:
                st.write("🤖 Bot:", answer)
            else:
                st.write("🤖 Bot: Saya belum tahu jawaban untuk itu.")
                # Tampilkan tombol "Ajukan Pertanyaan Ini" hanya jika chatbot tidak bisa menjawab
                if st.button("Ajukan Pertanyaan Ini", key="ajukan_button"):
                    user_questions_df = load_user_questions()
                    if st.session_state.user_input not in user_questions_df["question"].values:
                        new_question = pd.DataFrame({"question": [st.session_state.user_input]})
                        user_questions_df = pd.concat([user_questions_df, new_question], ignore_index=True)
                        save_user_questions(user_questions_df)
                        st.success("Pertanyaan Anda telah diajukan. Terima kasih!")
                        st.session_state.user_input = ""
